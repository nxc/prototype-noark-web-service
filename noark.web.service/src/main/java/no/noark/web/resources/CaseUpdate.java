package no.noark.web.resources;

import javax.ws.rs.Consumes;
import javax.ws.rs.GET;
import javax.ws.rs.PUT;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;
import javax.ws.rs.core.Context;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;
import javax.ws.rs.core.UriInfo;

import no.noark.database.Database;
import no.noark.model.CaseBean;
import no.noark.model.Link;
import no.noark.templates.TemplateParams;
import no.noark.templates.Templates;
import no.noark.web.helpers.ResponseHelper;

@Path("cases/{systemId}/update")
public class CaseUpdate {

	@GET
	@Produces(MediaType.TEXT_HTML)
	public Response caseUpdateUnsupported(@Context UriInfo uriInfo,
			@PathParam("systemId") String systemId) {

		CaseBean sak = Database.get().findBySystemId(systemId, CaseBean.class);

		String output = Templates.get().produce(
				"case-update.xml",
				new TemplateParams()
						.add("url", uriInfo.getAbsolutePath().toString())
						.add("method", "PUT")
						.add("caseFileId", sak.getCaseFileId())
						.add("caseTitle", sak.getCaseTitle())
						.add("caseOfficialTitle", sak.getCaseOfficialTitle()));

		return Response.status(405).entity(output).build();
	}

	@PUT
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response caseUpdate(@Context UriInfo uriInfo,
			@PathParam("systemId") String systemId, CaseBean updates) {

		updates.setSystemId(systemId);

		Database.get().update(updates);

		Link link = new Link("self", uriInfo.getBaseUri() + "cases/"
				+ updates.getSystemId());

		return ResponseHelper.generateResponse(uriInfo, link, false);
	}

}
